...
export async function rpc<T = unknown>(
  ...
  const frame = encodeFrame(
    { jsonrpc: '2.0', id: requestId, method, params, token },
    IPC_REQUEST_BYTES,
    'request',
  );
  return new Promise((resolve, reject) => {
    ...
    socket.on('data', (data) => {
      ...
      socket.end();
      try {
        const response = JSON.parse(buffer.slice(0, at)) as {
          id: string;
          result: T;
          error?: { message: string; data?: unknown };
        };
        if (response.id !== requestId) throw new Error('IPC response ID mismatch');
        if (response.error)
          throw (
