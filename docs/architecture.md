...
Модуль `insights` получает технические фазы через узкий `ExecutionObserver`. Runtime и провайдеры не зависят от файлового адаптера. `application` связывает отчёты с задачами и проектными проверками и очищает измерения при каскадном удалении. Журнал `activity/<runId>.jsonl` не содержит переписку; очереди и сводки ограничены независимо от основной истории. Подробнее: [измерения и стенд](execution-insights.md).
