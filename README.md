...
Текущая версия проекта — **0.2.0**. Runtime закреплён на **Node.js 24.21.0**, npm — **11.x**. Файлы запуска устанавливают Node в папку приложения, сохраняя системную установку.

В выпуске **0.2.0** можно отправлять сообщения работающей задаче через **Ctrl+W** или команду `message`. Сообщения сохраняются в очереди до следующего шага координатора. Исправлено восстановление дерева подзадач после сбоя; функции и методы исходного кода дополнены JSDoc на русском языке. [Описание выпуска и проверка поставки](docs/release-stability.md).
