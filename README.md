...
Текущая версия проекта — **0.5.0**. Runtime закреплён на **Node.js 24.21.0**, npm — **11.x**. Файлы запуска устанавливают Node в папку приложения, сохраняя системную установку.

В **0.5.0** проектный режим получил редактор и сравнение планов, список ожидающих решений, полный сохранённый вывод проверок, приёмку по доказательствам и экспорт Markdown/JSON. Первый проект настраивается через формы с сохраняемым черновиком. Для коротких запросов остаются обычные задачи. [Работа с проектами](docs/projects.md).
