...
Текущая версия проекта — **0.1.0**. Runtime закреплён на **Node.js 24.21.0**, npm — **11.x**. Файлы запуска устанавливают Node в папку приложения, сохраняя системную установку.
...
| `purge` / «Удалить навсегда»  | Вся беседа `sessionId`, включая скрытые этапы, потоки, артефакты, резервные копии, связанные знания и их внутренние экспорты |
...
Комментарии и краткий JSDoc пишутся по-русски. Изменение поведения сопровождается тестом воспроизведения; общие типы и вспомогательные функции располагаются рядом со своей функциональностью. Подробные границы — в [архитектуре](docs/architecture.md) и [сопровождении](docs/maintainability.md). [План развития](docs/architecture-roadmap.md) описывает будущие изменения отдельно от реализованных функций. Хранилище сейчас файловое и рассчитано на одного владельца; распределённый запуск не реализован.
...
Последний зафиксированный [аудит исходников](docs/source-bug-followup.md): **570/570 тестов в 86 файлах** на macOS arm64 и Linux arm64 с Node 24.21.0. [Отчёт с хешами и границами проверки](docs/source-followup-check.json). Это результат указанного прогона; он не заменяет проверку последующих изменений, Windows и реальных облачных подключений.
...
| Аудиты и исправленные ошибки               | [algorithm-audit.md](docs/algorithm-audit.md), [source-bug-audit.md](docs/source-bug-audit.md), [source-bug-followup.md](docs/source-bug-followup.md) |
