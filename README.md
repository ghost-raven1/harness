...
- Учебный проект без сети и API-ключей: `harness demo`.
- Сравнение файлов проекта, этапов и исправлений: «Изменения / До / После».
...
Текущая версия проекта — **0.7.0**. Готовые ZIP содержат **Node.js 24.21.0** и библиотеки: после распаковки установка зависимостей не требуется. Для разработки из исходников используются тот же Node и npm **11.x**.

В **0.7.0** появился раздел **«Работа специалистов»**: дерево агентов, поручения, смены ролей, фазы работы, ожидания и расход по запросам. Подтверждённые токены отделены от оценок и отсутствующих данных. Алгоритм делегирования сохранён. [Измерения, отчёты и сравнительный стенд](docs/execution-insights.md).

В **0.6.1** добавлены готовые поставки для четырёх платформ и индикатор работы CLI. Он различает ожидание ответа модели, получение текста и выполнение инструментов; известная длительность берётся из журнала. Разрешения, паузы и ошибки отображаются неподвижным состоянием. Анимация не создаёт дополнительных запросов к сервису или модели.

В **0.6.0** добавлены сохранение допустимых исходников, сравнения этапов и проверок, экспорт с построчными изменениями и учебное демо. Редактор планов, решения и приёмка по доказательствам из 0.5.0 сохраняются. [Работа с проектами](docs/projects.md), [изменения файлов и демо](docs/project-changes.md).

Чтобы попробовать без настройки модели, выполните `harness demo` после установки или выберите «Попробовать на учебном проекте» в главном меню. Демо использует отдельную временную папку; при выходе его данные и отчёты удаляются. Собственные проекты и подключения сохраняются.
...
Скачайте [готовый ZIP своей системы](https://github.com/ghost-raven1/harness/releases/latest): `windows-x64`, `linux-x64`, `macos-arm64` для Apple Silicon или `macos-x64` для Intel. Распакуйте целиком в папку, доступную для записи. Node.js и библиотеки уже внутри. Для работы облачной модели нужен аккаунт провайдера или Codex; учебное демо работает без сети и ключей.

Архив с суффиксом `source` предназначен для разработки: его первый запуск загружает зависимости и собирает приложение. [Состав и проверка готовой поставки](docs/portable-release.md).
...
1. Откройте файл запуска. Для знакомства без подключения модели выберите учебное демо или выполните `sh ./start.sh demo` (Windows: `"Запустить Harness.cmd" demo`).
...
Готовые архивы проверяются нативно для четырёх указанных платформ. Для других архитектур используйте исходную поставку с учётом [ограничений переносимости](docs/portability.md). Для первого знакомства есть [пошаговая инструкция](docs/getting-started.md) и [Начните здесь.txt](<Начните здесь.txt>).

## Работа с задачами

### Навигация и ввод

Один экран заменяет другой. Списки задач, открытая задача, знания, разрешения и настройки перечитывают изменяемые данные примерно раз в секунду. Выбор, вкладка и прокрутка сохраняются; временная потеря связи не стирает показанные данные. В активных экранах движущийся индикатор показывает текущую работу. Он перерисовывается локально четыре раза в секунду; при потере связи устаревшее состояние перестаёт анимироваться.
...
В карточке задачи или проекта откройте «Работа специалистов». Выберите агента, чтобы увидеть его поручение, текущую фазу, историю ролей, журнал измерений и полный результат по страницам. Проектный список разделяет этапы и попытки. Успешный ответ агента и прохождение проверок отображаются отдельно.

```sh
harness insights RUN_ID
harness projects insights PROJECT_ID --json
harness benchmark specialists --output ./specialists-offline
```

Офлайн-стенд выполняет 45 запусков: пять сценариев, три режима, три повтора. Он проверяет механизм измерений без сети. Для сравнения настоящей модели:

```sh
harness benchmark specialists --real --config ./config/harness.json --profile qwen --output ./specialists-qwen
```

До обращения к модели показываются сценарии и число запусков; требуется подтверждение. Стенд использует свои папки, настройки ролей и состояние, с отключённым обучением. Офлайн-цифры не доказывают эффективность реального провайдера. [Устройство и ограничения отчётов](docs/execution-insights.md).
...
| `exports/`                                                 | Markdown-экспорты уроков; `exports/projects/` — отчёты проектов                |
| `project-records/`, `project-index/`, `project-artifacts/` | История проектов, производные индексы и отпечатки проверенных файлов           |
...
| [insights](src/insights/)                                        | Компактные фазы исполнения, ограниченная очередь, сводки и страницы измерений                |
...
Для готового архива на машине целевой ОС и архитектуры:
...
Упаковщик устанавливает production-зависимости во временной папке сборки и добавляет runtime. Проверка распаковывает ZIP в новую папку и проходит офлайн-сценарий без системных Node/npm. Четыре нативных задания CI выпускают независимые ZIP и SHA256. [Подробности поставки](docs/portable-release.md).

Для архива исходников:
...
- [Работа специалистов и сравнительный стенд](docs/execution-insights.md)
