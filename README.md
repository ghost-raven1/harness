...
Текущая версия проекта — **0.2.1**. Runtime закреплён на **Node.js 24.21.0**, npm — **11.x**. Файлы запуска устанавливают Node в папку приложения, сохраняя системную установку.

В выпуске **0.2.1** исправлены гонки при запуске, отмене и продолжении задач, сохранение журнала при ошибках записи и завершение сервиса. Незавершённое восстановление файла можно проверить через CLI; подтверждённые исправления сохраняются при продолжении беседы. [Описание выпуска и проверка поставки](docs/release-stability.md).
